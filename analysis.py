...
def generate_current_ratio():
    df = pd.read_excel(r'./data/project.xlsx', sheet_name='Sheet1', skiprows=range(4, 58))
...
def generate_quick_ratio():
    df = pd.read_excel(r'./data/project.xlsx', sheet_name='Sheet1', skiprows=12, nrows=3)

    plt.figure(figsize=(3, 3))
    plt.pie(df['Quick Ratio'], labels=df['Year'], autopct='%1.1f%%', startangle=90, colors=["green","yellow","blue"])
...
def generate_net_profit_ratio():
    df = pd.read_excel(r'./data/project.xlsx', sheet_name='Sheet1', skiprows=54, nrows=3)
    ...
    plt.figure(figsize=(3, 3))
    plt.pie(df['Net Profit Ratio'], labels=df['Year'], autopct='%1.1f%%', startangle=90, colors=["green","blue","orange"])
...
def future_prediction_current_ratio():
    ...
    # Plot predicted data
    plt.plot(future_years.flatten(), future_predictions, color="red", linestyle="dashed", label="Predicted Data")
